...
**关键技术风险点**：
- Univer snapshot 体积（大表格嵌入 md 文件会膨胀）→ 可选外置 `.hy.assets/` 目录
...
| **M1 Sheet 块**hymd-blocks-sheet；Univer 只读+编辑；snapshot 往返嵌入/导出 sheet 块公式不丢失；Obsidian Sheet Plus 级体验 |                                                   |                                                            | 3 周 |
